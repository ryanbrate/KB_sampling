...
Run:
```
python3 article_count_by_year.py
```
"""
...
def main():
    ...
    # get a set of date windows of interest to query
    input_date_windows: list[str] = []

    year = 1870
    while year < 2010:
        input_date_windows.append(f"{year}-01-01 {year}-12-31")
        year+=1
    ...
    for url, response in tqdm(gen_threaded(urls, f=get_response, max_workers=50)):
    ...
    # save
    df.to_csv("article_counts_by_year.csv")
